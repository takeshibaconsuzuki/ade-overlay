...
/**
 * Ingests log records shipped by clients (the renderer) and re-emits them
 * through the server logger, so all logs are consolidated in one stream. Each
 * record is tagged `source: 'renderer'` to distinguish it from server logs.
 */
export function registerLogRoutes(server: FastifyInstance): void {
  const log = server.log.child({ source: 'renderer' })
  ...
  routes.route({
    ...
    handler: async (request) => {
      ...
      for (const record of records) {
        log[record.level](
          { ...record.bindings, ...record.fields, clientTime: record.time },
          record.msg ?? '',
        )
      }
